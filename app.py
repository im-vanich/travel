...
@app.route("/")
def index_render():
    return render_template("index.html", tours=tours, title=title, departures=departures)
...
def departures_render(departure):
    ...
    return render_template('departure.html', departures=departures, departure=departure,
                           tour_on_departure=tour_on_departure, title=title, min_price=min_price, max_price=max_price,
                           min_nights=min_nights, max_nights=max_nights)


@app.route('/tours/<int:id>/')
def tours_render(id):
    return render_template('tour.html', tours=tours, departures=departures, id=id, title=title)
